...
def get_filters():
    ...
    while city not in CITY_DATA.keys():
        ...
        city = input("select the city to get information ,type:\n chicago \n or new york city \n or washington)").lower()
        continue
...
def load_data(city, month, day):
    ...
    if month != 'all':
        ...
        dfm = df[df['S_month'] == month]
        

    # filter by day of week if applicable
    if day != 'all':
        # filter by day of week to create the new dataframe
        dfd = df[df['S_day'] == day.title()]
...
def station_stats(df):
    ...
    if mccs.empty:
        ...
        t_count=len(df.groupby(df['sct']).size().sort_values(ascending=False))
        mccs=df['sct'].iloc[0]
